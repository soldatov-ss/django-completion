...
Three docs-only fixes that improve discoverability and conversion. No code changes. All are independent and can be grabbed in any order.

---

## Issue 0.2.9-1 — Fix URL inconsistency in `zensical.toml`

**Goal:** `zensical.toml` uses underscores in both `site_url` and `repo_url`; every other reference in the project uses hyphens. Fix both entries so the docs site is built and linked with the correct URL.

**Root cause:** `zensical.toml` was written with `django_completion` (underscore). The actual GitHub repo slug and GitHub Pages URL are `django-completion` (hyphen). When the docs site is built, the canonical URL baked into `<link rel="canonical">` and sitemap entries will be the underscore variant, splitting any search-engine authority between two URLs that look identical to a human but are distinct to a crawler.

**Files:**
- `zensical.toml` — fix `site_url` and `repo_url`

**What to change:**

```toml
# before
site_url = "https://soldatov-ss.github.io/django_completion/"
repo_url = "https://github.com/soldatov-ss/django_completion"
repo_name = "soldatov-ss/django_completion"

# after
site_url = "https://soldatov-ss.github.io/django-completion/"
repo_url = "https://github.com/soldatov-ss/django-completion"
repo_name = "soldatov-ss/django-completion"
```

**Acceptance criteria:**
- `zensical.toml` contains no underscore variant of the project name.
- `grep "django_completion" zensical.toml` returns nothing.
- `grep "django-completion" zensical.toml` returns all three corrected lines.
- `uv run ruff check .` passes (no Python files touched).
- `uv run ty check` passes.

**Notes:**
- This must be fixed before any community promotion. Links posted in Reddit/LinkedIn/HN posts will point to the hyphen URL; if the docs site announces a different canonical URL, Google indexes the underscore variant and the hyphen variant accumulates no authority.
- No other files need changing — `pyproject.toml`, `README.md`, and `PROMOTION.md` already use hyphens throughout.

---

## Issue 0.2.9-2 — Add `how_it_works.md` and `troubleshooting.md` to the docs nav

**Goal:** Both docs pages exist and are fully written, but `zensical.toml` only lists four pages in the nav. `how_it_works.md` and `troubleshooting.md` are invisible to readers who enter from the sidebar, and their URLs are not linked from any other page — so search engines either never discover them or assign them no authority.

**Root cause:** The nav in `zensical.toml` was not updated when the two pages were written. The pages themselves need no content changes.

**Files:**
- `zensical.toml` — add two entries to `nav`
...
**Acceptance criteria:**
- `zensical.toml` nav contains exactly six entries in the order shown above.
- `how_it_works.md` and `troubleshooting.md` are positioned between Usage and API Reference.
- No content changes to either doc page.
- `uv run ruff check .` passes.
- `uv run ty check` passes.

**Notes:**
- `how_it_works.md` is the densest technical page in the docs — cache lifecycle, shell hook mechanism, AppConfig hook, refresh, uninstall. This is precisely the content that captures long-tail search queries like "django-completion cache not updating" or "manage.py tab completion not working zsh". Not linking it from the nav means users and crawlers only reach it through external links (if any).
- `troubleshooting.md` captures problem-aware search queries ("django autocomplete unknown command") that have high conversion intent — a developer who finds this page already wants the tool to work.
- Ordering: How it works after Usage (readers who want more depth after using it), Troubleshooting before API Reference (most readers need it earlier than the full reference).

---

## Issue 0.2.9-3 — Add comparison page: django-completion vs Django's built-in completion

**Goal:** Add `docs/comparison.md` — a focused page that explains what Django's built-in shell completion offers, where it stops, and what django-completion adds. Link it from `docs/index.md`.

**Context:** Django ships its own completion mechanism: a bash script (`django-admin.bash`) and argparse-based completion hooks. These are real, functional, and already installed by many package managers. Any developer who searches for "django tab completion" or "django manage.py autocomplete" will find Django's built-in option before django-completion. Without a comparison page, the evaluation question — "do I need a third-party package if Django already has completion?" — has no answer in the docs. This is the primary conversion blocker for evaluating developers, and it is also the content that will rank for comparison-intent queries.

This is a docs-only implementation of GitHub Issue #18 ("Comparison table with Django's built in completion").

**Files:**
- `docs/comparison.md` — new page
- `docs/index.md` — add one-line link to the comparison page
- `zensical.toml` — add `comparison.md` to the nav

**What to implement:**

`docs/comparison.md` must cover these four sections:

**1. What Django's built-in completion provides**

Django ships `django-admin.bash` (typically installed via your OS package manager or the `django-bash-completion` script). It completes:
- Top-level management command names after `django-admin` and `manage.py`
- Option flags for commands (via argparse's built-in completion support)

Setup requires manually sourcing a script or relying on system-wide bash-completion integration. There is no built-in zsh support.

**2. Where the built-in completion stops**

The built-in completion is generic — it reads argparse metadata but has no knowledge of your project's runtime state:
- It does not complete app labels because those come from `INSTALLED_APPS`, which requires Django to be configured
- It does not complete migration names because those come from the filesystem and depend on per-app migration modules
- `migrate <TAB>` shows all app labels (or none), not just the apps that actually have migrations
- `migrate accounts <TAB>` shows nothing — there is no built-in second-level migration name completion
- It has no concept of local vs pip origin for app labels
- Script updates require manual re-sourcing; there is no auto-refresh

**3. What django-completion adds**

Present this as a table. Exact Markdown:
...
| `migrate` app labels (only apps with migrations) | No | Yes |
| Migration names after `migrate <app>` | No | Yes |
| `zero` target for `migrate` | No | Yes |
| Migration names for `sqlmigrate` | No | Yes |
| `makemigrations` — local apps only | No | Yes |
| `showmigrations` — apps with migrations | No | Yes |
| zsh support with descriptions | No | Yes |
| `uv run python manage.py` invocation | No | Yes |
| Single install command | No | Yes |
| Auto-refresh after each command | No | Yes |
| Script currency detection (`status`) | No | Yes |
```

**4. When to use each**

- Use Django's built-in if you only need command-name and flag completion and prefer zero additional dependencies.
- Use django-completion if you work with migrations regularly, have many apps, or want zsh support with descriptions.
- Both can coexist: django-completion's shell hook takes precedence when a `.django-completion-cache.json` is found; Django's built-in handles `django-admin` (which django-completion does not support).

**Add to `docs/index.md`:**

After the existing "Why it exists" section, add one line linking to the comparison:

```markdown
See [Comparison with Django's built-in completion](comparison.md) for a feature-by-feature breakdown.
```

**Add to `zensical.toml` nav** (after the entry added in Issue 0.2.9-2):

```toml
{ "Comparison" = "comparison.md" },
```

Position: between How it works and Troubleshooting, or between Home and Installation — either is acceptable. Between Installation and Usage is the recommended position so evaluating readers see it early.

**Acceptance criteria:**
- `docs/comparison.md` exists and contains all four sections listed above.
- The feature comparison table contains at least the 13 rows shown above.
- `docs/index.md` links to `comparison.md`.
- `zensical.toml` nav includes `comparison.md`.
- No claims in the comparison page are unsupported by current behavior — verify against `_complete.py` and the current test suite before writing.
- `uv run ruff check .` passes.
- `uv run ty check` passes.

**Notes:**
- The tone of the comparison must be factual, not adversarial. Django's built-in completion is a correct and appropriate tool for many workflows. The comparison's job is to answer the evaluation question honestly, not to win a feature-count contest.
- The page title in the nav should be "Comparison" (short) not "vs Django built-in" — the latter reads as marketing copy in a sidebar.
- Do not add a `noindex` meta tag. This page should be indexed. It is the most likely page to rank for comparison-intent queries ("django tab completion vs", "django-completion vs argparse completion") and to earn links from Reddit/HN answers.
- Verify the "Django built-in" column claims before writing. The built-in script behavior differs between Django versions and OS package installations; write only what is reliably true across Django 4.2+.
